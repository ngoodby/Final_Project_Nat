...
It looks like our variables fit a normal distribution overall. However, the fit is not as good at the more extreme values ("heavy tails"). This is somewhat disconcerning for our ability to make inferences based on the assumption of having a normal distribution.

Let's do a heterscadisticity test to look at error variance across out data. If the output of the ncvTest function is non-significant, the error variance changes with the level of the response. Let's run the test:
```{r}
ncvTest(test3)
```
So, indeed, the error variance does change with the level of response.
...
```
Our adjusted R-squared value is still about the same at 0.5405. 
