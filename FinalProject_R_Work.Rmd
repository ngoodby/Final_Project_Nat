...
summary(test1)
```
Those two variables show *significant correlation* however the adjusted R-squared value is only around 0.25, so the influence doesn't seem to be very strong.
```{r}
...
summary(test3)
```
So both GDP per capita and urbanization both predict CO2 emissions to some extent their loose correlation suggests that there are other factors at play as well. 
